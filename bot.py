...
def fetch_sealagom_navtex():
    try:
        ...
        soup = BeautifulSoup(r.text, "html.parser")
        msgs = []
        pattern = re.compile(r"(\d{4}/\d{2}.*?)(?=\d{4}/\d{2}|$)", re.DOTALL)
        for block in pattern.findall(soup.get_text()):
            block = block.strip()
            if block:
                msgs.append(block)
        return msgs
...
def check_navtex():
    ...
    for m in msgs:
        ...
        if len(coords)==1:
            bot.send_location(CHAT_ID, coords[0][0], coords[0][1])
        elif len(coords)>1:
            for lat, lon in coords:
...
def main():
    ...
    while True:
        try:
            check_gov()
            check_metarea()
            check_navtex()  # автопроверка NAVTEX каждые 15 минут
